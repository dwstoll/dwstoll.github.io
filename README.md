...
Zusätzlich muss definiert werden, für welchen Bereich des HTML-Dokuments der erstellte Controller Anwendung finden soll. Dies erfolgt über das AngularJS-Attribut `ng-controller`, das in diesem Fall im `body-Tag` definiert wird. Zusätzlich wird das input-Eingabefeld wieder, wie im oberen Beispiel, mit dem Attribut `ng-model` und dem Wert `suche`, erweitert. 

Anschließend erfolgt mittels den HTMl-Tags `ul` und `li` eine Ausgabe der Adressbuchdaten. Beim `li-Tag` kommt das AngularJS-Attribut `ng-repeat` zu tragen, was einer Schleife ähnelt. Hierbei wird jedes Objekt aus dem im Controller definierten Array `$scope.benutzer`  durchlaufen und jeweils der lokalen Variable `person` zugewiesen. Da im `body-Tag` definiert wurde, dass sich der gesamte `body-Bereich` durch AdressbuchController verwaltet wird, erfolgt ein **Zugriff** auf `$scope.benutzer` lediglich durch `benutzer`. 
Auf die einzelnen Attribute des $scope.benutzer Arrays, wie Vorname und Name, kann über die lokale Variable person mittels Punktnotation zugegriffen werden, wobei dies wieder als **Expressions** in geschweiften Klammern definiert wird.
Somit wird durch `ng-repeat' und den entsprechenden Expressions alle Adressbuchdaten aller Personen ausgegeben. Damit eine Einschränkung durch das Eingabefeld erfolgen kann, muss das `ng-repeat-Attribut` lediglich um die AngularJS-`Filter-Anweisung` mit Angabe des Eingabefeldes, folglich `filter:suche`, erweitert werden.

Dementsprechend erfolgt über das **$scope-Objekt** bei komplexeren Beispielen eine zweidimensionale Datenbindung, die eine automatische Verbindung zwischen **Controller und View** ermöglicht. 
...
Die **erste Möglichkeit**, die im Folgenden Code minimalistisch skizziert ist, geht von der Tatsache aus, dass ein **Adressbuch-Objekt** existiert und dieses über die `Funktion sucheAdressbuch` verfügt. Da die Adressdaten in einer Datenbank liegen, muss auf ein Datenbank-Objekt zugegriffen werden. In diesem Fall wird daher in der `Funktion sucheAdressbuch` über den `new-Parameter` ein neues Datenbank-Objekt angelegt. Demzufolge erfolgt hierbei die Instanziierung eines neuen Objekts und der Aufbau der Abhängigkeit **innerhalb einer Funktion** eines Objekts, das wiederum das zu instanziierende Objekt benötigt.

Der **andere Ansatz**, auch **Dependency Injection** genannt,  verfolgt das Ziel, dass das Objekt der `sucheAdressbuch-Funktion` von außen als Übergabeparameter übergeben wird und demzufolge eine Instanziierung des Objekts nicht in der Funktion erfolgt, die das Objekt benötigt. 
Dieses Verfahren kommt bei AngularJS zum Tragen, was unter anderem bereits bei den Beispielen mit der `$scope-Variable` zu sehen war. Bei der Definition eines Controllers wird die `$scope-Variable` lediglich als Parameter übergeben und es ist demzufolge keine Instanziierung innerhalb des Controllers erforderlich. Bei Objekten, die Bestandteil vom AngularJS-Framework sind, zum Beispiel `$scope`, erfolgt eine automatische Instanziierung ohne gesonderte Definition durch den Programmierer.
...
Durch die Verfolgung des Dependency Injection Ansatzes wird eine **bessere Testbarkeit** erreicht, indem die reine Funktionalität von Funktionen, unabhängig von der Instanziierung von anderen Objekten, getestet werden kann, in dem Funktionen Testobjekte übergeben werden können.  
